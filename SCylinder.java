package Objects;	//with reference to SSphere.java
...
public class SCylinder extends SObject {
	...
	protected void genData() {
		...
		for(i=1;i<stacks;i++){
			for(j=0;j<=slices;j++){
				normals[3*k] = sin(PI/2)*cos(deltaLong*j);	//x-axis set phi 90 degree
				normals[3*k+1] = sin(PI/2)*sin(deltaLong*j);	//y-axis set phi 90 degree
				normals[3*k+2] = cos(deltaLat*i);			//z-axis
